# Contributing: обновление иконок в icons ADMIRAL DS
...
Если нет новых — шаг пропускается.
...
Перед snapshot-тестами стоит запустить полную проверку:

```bash
npm run check:full
```

Эта команда запускает prettier, lint, проверку package exports и e2e/snapshot-тесты.

Если нужно запустить только snapshot-тесты:
...
5. Подготовить **3 коммита**
6. Собрать Storybook через `npm run storybook:build`
7. Проверить Storybook через `npm run storybook:preview`
8. Создать PR
9. Принять PR
